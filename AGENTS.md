...
- **Health Monitor UI**: Environment filtering (All, Production, Staging, Unhealthy) and sorting must be implemented via dual `IndustrialTabs` at the route level. `ProductSection` headers must use high-density technical typography with `Box` icon and semantic status badges at 20% opacity.
- **Novedades UI**: Content must be encapsulated in a `bg-muted/10` container with `rounded-xl` geometry and `border-border/40`. Header must use the `Newspaper` icon.
- **Table Standard**: Internal filter bars must use `bg-muted/40` and apply high-density technical typography (`text-[10px] font-bold uppercase`) to buttons.
