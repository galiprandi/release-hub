...
- **Verificación**: Build zero-warning, 291 tests exitosos, lint sin problemas nuevos. Verificado contra cluster real (los pods exponen GIT_COMMIT heredado del ReplicaSet).
...
| 9 | **Prohibido hardcodear nombres propiedad intelectual de Cencosud** (orgs, productos, namespaces, dominios) en código, tests, mocks o docs. Única excepción: `seki` (y su endpoint en el proxy de `vite.config.ts`). Usar genéricos: `acme-org`, `my-product`, `demo-*`, `example.com`. Orgs de búsqueda se resuelven dinámicamente via `getRepoSearchScope()` (`src/api/githubSearch.ts`). | — |
...
- **Header Standard (PageLayout)**: Todos los módulos deben seguir el estándar de headers definido en `DESIGN.md` §Header Standard. Title siempre JSX con icon `w-4 h-4 text-primary`. Filtros via `CollectionDropdown`, nunca `IndustrialTabs` ni `select` nativo. Divider `w-px h-6 bg-border` entre dropdowns y search. Refresh via `refreshFn` prop del `PageLayout`, no en `actions`. Module-specific actions como array de `ActionButton` con inline action, `showLabel={false}`, `size="md"`, sin className overrides. No usar `ACTION_DEFINITIONS` en el header.
...
- **Docker UI**: Status filtering and `ContainerSearch` in the `PageLayout` header using `CollectionDropdown` and a dedicated search input. Cells use semantic dots (`StatusCell`) and hover-to-reveal patterns (`ActionsCell`).
- **Fetcher UI**: Filtering, sorting, and text search (`q` param) via `CollectionDropdown` and a header search input. `UrlCell` uses a double-line pattern: domain (`text-xs text-muted-foreground`), path (`text-sm font-medium`).
...
- **GitHub UI**: Collection navigation is a `DropdownMenu` (Favoritos + proyectos) in the `PageLayout` header, synced with `tab`. Dashboard-level filtering uses `CollectionDropdown` synced with `filter`. Metadata cells use `text-xs font-medium text-muted-foreground`. Organization groups are collapsible.
- **Health Monitor**: Primary environment filtering (Production, Staging, Unhealthy) in the PageLayout header using `CollectionDropdown`. Product sections use `border border-border rounded-md bg-card` containers with `Box` icons.
