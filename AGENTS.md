...
- **Docker UI**: Status filtering is promoted to the `PageLayout` header using `IndustrialTabs`, synced with `status` search parameter. Header includes the technical `Boxes` icon. `StatusCell` includes semantic dots and high-density badges. `PortsCell` uses an integrated high-density selector. `ActionsCell` operations are revealed only on hover to maintain layout focus. Empty states utilize technical typography with `tracking-[0.2em]`.
