...
- **GitHub UI**: Collection navigation is a `DropdownMenu` (Favoritos + proyectos) in the `PageLayout` header, synced with `tab`. Dashboard-level filtering uses `IndustrialTabs` synced with `filter`. Metadata cells use `text-xs font-medium text-muted-foreground`. Organization groups are collapsible.
