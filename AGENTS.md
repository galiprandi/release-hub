...
- **Shell Hardening**: Todo comando CLI (gh, docker, kubectl, curl) debe usar `runCommand` con un array de argumentos. Parámetros dinámicos deben pasar por `quote()` o ser parte del array de `runCommand` (que internamente usa `joinArgs`). Prohibido concatenar strings sin escape para comandos. Para entradas complejas (JSON), usar el parámetro `stdin` de `runCommand` en lugar de Heredocs o tuberías de shell.
