...
- **Path Traversal & Parameter Validation**: El middleware de Vite (`vite.config.ts`) y de terminal (`terminalMiddleware.ts`) deben aplicar validaciones estrictas vía regex para todos los parámetros de entrada (`action`, `repo`, `name`, `namespace`, `context`, `container`). Se prohíbe explícitamente el uso de secuencias `..` en cualquier parámetro de ruta para evitar vulnerabilidades de path traversal. El endpoint `execHandler` bloquea activamente (400) cualquier argumento que contenga metacaracteres de shell (`;&|><`).
