...
- **Path Traversal & Parameter Validation**: El middleware de Vite (`vite.config.ts`) y de terminal (`terminalMiddleware.ts`) deben aplicar validaciones estrictas vía regex para todos los parámetros de entrada (`action`, `repo`, `name`, `namespace`, `context`, `container`). Se prohíbe explícitamente el uso de secuencias `..` en cualquier parámetro de ruta para evitar vulnerabilidades de path traversal. Se aplican estándares RFC 1123 para recursos de Kubernetes y regex específicos para Docker y contextos. El endpoint `execHandler` bloquea activamente (400) cualquier argumento que contenga metacaracteres de shell (`;&|><`).
- **SSRF Protection**: El handler `health-proxy` implementa una denegación estricta de solicitudes dirigidas a direcciones IP de loopback (incluyendo variantes IPv6 y mapeos), redes privadas (RFC 1918), link-local (169.254.169.254) y dominios internos para prevenir ataques de Server-Side Request Forgery en el entorno de desarrollo.
...
- **Type Safety Strategy**: Se prohíbe el uso de `any`. Se prioriza el uso de interfaces explícitas y `unknown` para asegurar la integridad de los datos en tiempo de compilación. La integración con WebMCP debe seguir un esquema de validación estricto en los handlers de ejecución. La gestión de colecciones de usuario y el manejo de eventos globales deben estar estrictamente tipados.
