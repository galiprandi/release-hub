...
export function SetupCard({
	...
	if (isInstalled) {
		return (
			<div className="flex items-start gap-3 text-success text-sm border border-success/40 rounded-md p-4 bg-success/10 shadow-sm transition-all hover:bg-success/20">
				<CheckCircle className="w-5 h-5 mt-0.5" />
				<div className="flex-1">
					<div className="flex items-center gap-2">
						<p className="text-xs font-medium">{name}</p>
						<span className="px-1.5 py-0.5 rounded-md bg-success/20 border border-success/40 text-xs font-medium">
							Instalado
						</span>
					</div>
