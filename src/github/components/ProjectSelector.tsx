		...
		<div className="relative" ref={containerRef}>
			...
			{isOpen && (
				<div role="listbox" className="absolute top-full right-0 md:left-0 mt-1 w-72 bg-background border rounded-lg shadow-lg z-50 py-1 animate-in fade-in zoom-in-95 duration-150">
					...
					{projects.map(p => {
						...
						return <button key={p.id} type="button" role="option" aria-selected={inP} onClick={() => { void (inP ? removeRepoFromProject(p.id, repo) : addRepoToProject(p.id, repo)); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-muted transition-colors text-left focus-visible:ring-2 focus-visible:ring-primary/30 focus-visible:outline-none focus-visible:ring-offset-1 rounded-sm">
					...
					<div className="border-t mt-1 pt-1">
						<button
							...
							className="w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-muted transition-colors text-left text-primary focus-visible:ring-2 focus-visible:ring-primary/30 focus-visible:outline-none focus-visible:ring-offset-1 rounded-sm"
						>
			...
			>
				<form onSubmit={handleCreate} className="space-y-4">
					<div>
						<label htmlFor="project-name" className="block text-xs font-medium text-muted-foreground mb-2">
							Nombre del proyecto
						</label>
						<input
							id="project-name"
					...
					<div>
						<label htmlFor="project-desc" className="block text-xs font-medium text-muted-foreground mb-2">
